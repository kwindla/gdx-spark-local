"""HuggingFace dataset download and filtering for ASR evaluation."""
...
def generate_sample_id(dataset_index: int, audio_id: str) -> str:
    ...
    # Create a hash-based UUID from the dataset index and audio ID
...
class DatasetDownloader:
    """Downloads and processes the smart-turn dataset for ASR evaluation."""
    ...
    async def download_and_prepare(
        ...
        # Check if we already have samples (only when not using offset)
        ...
        logger.info(f"Existing samples: {existing_count}, downloading indices {self.offset}-{target_total - 1}")
        ...
        filtered = dataset.filter(
            lambda x: x["language"] == "eng" and x["synthetic"] is False
        )

        if progress_callback:
            progress_callback(2, 4, "Shuffling and selecting samples...")

        # Shuffle with seed for reproducibility and take num_samples
        shuffled = filtered.shuffle(seed=self.seed)

        # Calculate the end index (offset + num_samples)
        end_index = self.offset + self.num_samples

        if progress_callback:
            progress_callback(3, 4, f"Processing {self.num_samples} samples (indices {self.offset}-{end_index - 1})...")
        ...
        for i, item in enumerate(shuffled):
            ...
            if progress_callback and processed_count % 50 == 0:
                progress_callback(
                    3, 4, f"Processing sample {processed_count + 1}/{self.num_samples} (index {i})..."
                )
            ...
            if processed_count % 100 == 0:
                logger.info(f"Processed {processed_count}/{self.num_samples} samples (at index {i})")
    ...
    async def _process_sample(
        self, item: dict, dataset_index: int
    ...
    async def get_sample_audio(self, sample: AudioSample) -> bytes:
        """Load audio bytes for a sample.

        Args:
            sample: AudioSample to load
