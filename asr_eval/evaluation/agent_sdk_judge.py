"""Agent SDK WER Judge - Multi-turn reasoning-based WER evaluation.

Uses Claude Opus with tool use for multi-step WER calculation:
1. LLM normalizes both texts using few-shot examples
2. LLM performs word-level alignment
3. LLM counts errors and verifies work
4. Programmatic tool calculates final WER

Full reasoning traces are stored for debugging and auditing.
Samples where Agent SDK disagrees with Gemini are flagged for human review.
"""
...
AGENT_SDK_SYSTEM_PROMPT = '''You are an expert ASR evaluator for a conversational AI system. Your task is to calculate the Semantic Word Error Rate (WER) - counting ONLY transcription errors that would impact how an LLM agent understands and responds to the user.
        ...
        "properties": {
            ...
            "errors": {
                ...
                "items": {
                    "type": "object",
                    "properties": {
                        "type": {"type": "string", "enum": ["substitution", "deletion", "insertion"]},
                        "reference": {"type": "string", "description": "Reference word (null for insertion)"},
                        "hypothesis": {"type": "string", "description": "Hypothesis word (null for deletion)"},
                        "position": {"type": "integer", "description": "Position in alignment"},
                    },
                },
            },
        },
...
class AgentSDKJudge:
    """Multi-turn WER judge using Claude with tool use."""

    # Flag ANY difference between Agent SDK and Gemini for human review
    ...
    async def evaluate(
        ...
        Returns:
            Tuple of (result dict, AgentSDKTrace with full reasoning)
        """
...
Show your work clearly, then call calculate_wer with your verified counts."""
        ...
        while num_turns < max_turns:
            ...
            try:
                response = await self.client.messages.create(
                    model=self.model,
                    max_tokens=4096,
                    system=AGENT_SDK_SYSTEM_PROMPT,
        ...
        trace = AgentSDKTrace(
            ...
            service_name=ServiceName.NVIDIA_PARAKEET,  # Will be set by caller
            ...
            alignment=None,  # Could be extracted from reasoning
            agent_sdk_wer=result["wer"] if result else 0.0,
    ...
    def _empty_result(self, session_id: str, start_time: float) -> tuple[dict, AgentSDKTrace]:
    ...
    async def evaluate_sample(
        ...
        """Evaluate a single sample and update its metrics with agent_sdk_wer.

        Args:
            sample_id: Sample identifier
            service_name: Service that produced the transcription

        Returns:
            Updated EvaluationMetrics or None if data not available
        """
        await self.db.initialize()

        # Get existing metrics
        metrics = await self.db.get_metrics(sample_id, service_name)
        if not metrics:
            logger.warning(f"No metrics found for {sample_id}/{service_name.value}")
            return None

        # Get ground truth and transcription
        gt = await self.db.get_ground_truth(sample_id)
        if not gt:
            logger.warning(f"No ground truth for {sample_id}")
            return None

        transcription = await self.db.get_transcription(sample_id, service_name)
        if not transcription:
            logger.warning(f"No transcription for {sample_id}/{service_name.value}")
            return None

        # Evaluate with Agent SDK
        result, trace = await self.evaluate(gt.text, transcription.transcribed_text)
        ...
        # Store the trace
        await self.db.insert_agent_sdk_trace(trace)

        # Update metrics with agent_sdk_wer
        updated_metrics = EvaluationMetrics(
            ...
            semantic_wer=metrics.semantic_wer,
            semantic_errors=metrics.semantic_errors,
            gemini_wer=metrics.gemini_wer,
            agent_sdk_wer=result["wer"],
            timestamp=datetime.utcnow(),
        )

        await self.db.insert_metrics(updated_metrics)

        # Check for disagreement with Gemini and flag for human review
        if metrics.gemini_wer is not None:
            disagreement = abs(result["wer"] - metrics.gemini_wer)
            if disagreement > self.DISAGREEMENT_THRESHOLD:
                review = HumanReview(
                    sample_id=sample_id,
                    service_name=service_name,
                    review_status=ReviewStatus.PENDING,
                    flagged_reason=f"Agent SDK WER ({result['wer']:.2%}) differs from Gemini WER ({metrics.gemini_wer:.2%})",
                    agent_sdk_wer=result["wer"],
                    gemini_wer=metrics.gemini_wer,
                    wer_disagreement=disagreement,
                )
                await self.db.insert_human_review(review)
                logger.info(
                    f"Flagged {sample_id} for human review: "
                    f"Agent SDK={result['wer']:.2%} vs Gemini={metrics.gemini_wer:.2%}"
                )
    ...
    async def evaluate_all(
        ...
        """Evaluate all samples for a service with Agent SDK WER.

        Args:
            service_name: Service to evaluate
            progress_callback: Optional callback(current, total, sample_id)
            skip_existing: Skip samples that already have agent_sdk_wer
            limit: Maximum number of samples to evaluate

        Returns:
            List of updated EvaluationMetrics
        """
        await self.db.initialize()

        # Get all metrics for service
        all_metrics = await self.db.get_all_metrics_for_service(service_name)

        if not all_metrics:
            logger.warning(f"No metrics found for {service_name.value}")
            return []

        # Filter to those without agent_sdk evaluation if skip_existing
        if skip_existing:
            pending = [m for m in all_metrics if m.agent_sdk_wer is None]
        else:
            pending = all_metrics
        ...
        if not pending:
            logger.info(f"All samples already have Agent SDK WER for {service_name.value}")
            return all_metrics

        logger.info(f"Evaluating {len(pending)} samples for {service_name.value}")

        results = []
        for i, metrics in enumerate(pending):
            if progress_callback:
                progress_callback(i, len(pending), metrics.sample_id)

            try:
                updated = await self.evaluate_sample(metrics.sample_id, service_name)
                if updated:
                    results.append(updated)
                    logger.info(
                        f"[{i+1}/{len(pending)}] {metrics.sample_id}: "
                        f"agent_sdk_wer={updated.agent_sdk_wer:.2%}"
                    )
            except Exception as e:
                logger.error(f"Error evaluating {metrics.sample_id}: {e}")

        return results

    async def compute_pooled_wer(
        ...
        # Filter to valid metrics with agent_sdk_wer
        valid = [m for m in all_metrics if m.agent_sdk_wer is not None and m.agent_sdk_wer < float("inf")]

        if not valid:
            return {}

        # Sum errors and reference words from traces
        ...
        for m in valid:
            trace = await self.db.get_agent_sdk_trace(m.sample_id, service_name)
            if trace:
                total_substitutions += trace.substitutions
                total_deletions += trace.deletions
                total_insertions += trace.insertions
                total_reference_words += trace.reference_words

        if total_reference_words == 0:
            return {}

        pooled_wer = (total_substitutions + total_deletions + total_insertions) / total_reference_words
...
async def evaluate_agent_sdk_wer(
    service_name: ServiceName,
    progress_callback: Optional[callable] = None,
    limit: Optional[int] = None,
) -> list[EvaluationMetrics]:
    """Convenience function to run Agent SDK WER evaluation.
    ...
    Returns:
        List of EvaluationMetrics with agent_sdk_wer
    """
    judge = AgentSDKJudge()
    return await judge.evaluate_all(service_name, progress_callback, limit=limit)
