"""Gemini 3.0 Flash transcription for ground truth generation.
...
from asr_eval.models import AudioSample, GroundTruth
...
class GeminiTranscriber:
    """Generates ground truth transcriptions using Gemini 3.0 Flash with reasoning."""
    ...
    async def transcribe_sample(self, sample: AudioSample) -> Optional[GroundTruth]:
        ...
        try:
            ...
            # Generate transcription using new google-genai API
    ...
    async def transcribe_batch(
        ...
        for i, sample in enumerate(samples):
            ...
            if gt:
                ...
                logger.info(
                    f"[{i+1}/{len(samples)}] Transcribed: {gt.text[:50]}..."
                    if len(gt.text) > 50
                    else f"[{i+1}/{len(samples)}] Transcribed: {gt.text}"
                )
            else:
                logger.warning(f"[{i+1}/{len(samples)}] Failed to transcribe sample {sample.sample_id}")
    ...
    async def generate_all_ground_truth(
        ...
        return await self.transcribe_batch(
            samples, progress_callback=progress_callback
        )
...
async def generate_ground_truth(
    ...
    """Convenience function to generate ground truth transcriptions.

    Args:
        batch_size: Optional limit on number of samples to process
        progress_callback: Optional progress callback
    ...
    return await transcriber.transcribe_batch(
        samples, progress_callback=progress_callback
    )
