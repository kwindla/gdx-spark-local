"""Direct ElevenLabs adapter using REST API for batch transcription."""
...
class ElevenLabsDirectAdapter(BaseSTTAdapter):
    """Direct ElevenLabs adapter using the REST Speech-to-Text API."""
    ...
    async def transcribe(self, sample: AudioSample) -> TranscriptionResult:
        """Transcribe an audio sample using ElevenLabs Speech-to-Text API."""
        ...
        try:
            ...
            form_data.add_field('model_id', self.model)
            form_data.add_field('language_code', self.language)
