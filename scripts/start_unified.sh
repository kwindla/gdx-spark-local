...
# Start TTS server (Magpie TTS via NeMo)
...
# Wait for ASR/TTS to be ready before starting LLM
# This ensures ASR/TTS claim their GPU memory first (warmup runs during startup)
if [ "$ENABLE_LLM" = "true" ]; then
    if [ -n "$ASR_PID" ]; then
        echo ""
...
if [ "$ENABLE_LLM" = "true" ]; then
    ...
    case "$LLM_MODE" in
        llamacpp-q8)
            ...
            ;;
        llamacpp-q4)
            echo "  Mode: llama.cpp Q4 (lower VRAM usage)"
            llama-server \
                -m "${LLAMA_MODEL}" \
                --host 0.0.0.0 \
                --port 8000 \
